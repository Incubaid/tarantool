...
-- Modified by Iwan Budi Kusnanto (2016) to run on Tarantool & Luajit environment
...
local function err()
    local code = ossl.ERR_get_error()

    local err = ossl.ERR_reason_error_string(code)
...
function _M.new(self, opts)
    ...
    if opts.public_key then
        key = opts.public_key
        read_func = ossl.PEM_read_bio_RSAPublicKey
        is_pub = true

    elseif opts.private_key then
        key = opts.private_key
        read_func = ossl.PEM_read_bio_RSAPrivateKey

    else
        return nil, "not found public_key or private_key"
    end

    local bio_method = ossl.BIO_s_mem()
    local bio = ossl.BIO_new(bio_method)
    ffi_gc(bio, ossl.BIO_vfree)

    local len = ossl.BIO_puts(bio, key)
    if len < 0 then
        return err()
    end
    ...
    ffi_gc(rsa, ossl.RSA_free)

    -- EVP_PKEY
    local pkey = ossl.EVP_PKEY_new()
    ffi_gc(pkey, ossl.EVP_PKEY_free)
    if ossl.EVP_PKEY_set1_RSA(pkey, rsa) == 0 then
        return err()
    end

    --EVP_PKEY_CTX
    local ctx = ossl.EVP_PKEY_CTX_new(pkey, nil)
    if ffi_cast("void *", ctx) == nil then
        return err()
    end
    ffi_gc(ctx, ossl.EVP_PKEY_CTX_free)

    -- md_ctx init for sign or verify; if signature algorithm is seted
    if opts.algorithm then
        ossl.OpenSSL_add_all_digests()

        md = ossl.EVP_get_digestbyname(opts.algorithm)
        if ffi_cast("void *", md) == nil then
            return nil, "Unknown message digest"
        end

        md_ctx = ossl.EVP_MD_CTX_create()
    end

    -- ctx init for encrypt or decrypt
    -- default for encrypt/decrypt if nothing is set
    if opts.padding or not opts.digest then
        local init_func = is_pub and ossl.EVP_PKEY_encrypt_init
                            or ossl.EVP_PKEY_decrypt_init
        if init_func(ctx) <= 0 then
            return err()
        end

        if ossl.EVP_PKEY_CTX_ctrl(ctx, EVP_PKEY_RSA, -1, EVP_PKEY_CTRL_RSA_PADDING,
                opts.padding or PADDING.RSA_PKCS1_PADDING, nil) <= 0 then
            return err()
        end
    end

    local size = ossl.EVP_PKEY_size(pkey)
    return setmetatable({
            pkey = pkey,
            size = size,
...
function _M.decrypt(self, str)
    ...
    if ossl.EVP_PKEY_decrypt(ctx, nil, len, str, #str) <= 0 then
        return err()
    end

    local buf = self.buf
    if ossl.EVP_PKEY_decrypt(ctx, buf, len, str, #str) <= 0 then
        return err()
    end
...
function _M.encrypt(self, str)
    ...
    local len = ffi_new("size_t [1]")
    if ossl.EVP_PKEY_encrypt(ctx, nil, len, str, #str) <= 0 then
        return err()
    end

    local buf = self.buf
    if ossl.EVP_PKEY_encrypt(ctx, buf, len, str, #str) <= 0 then
        return err()
    end
...
function _M.sign(self, str)
    ...
    if ossl.EVP_DigestInit(md_ctx, self.md) <= 0 then
        return err()
    end

    if ossl.EVP_DigestUpdate(md_ctx, str, #str) <= 0 then
        return err()
    end

    local buf = self.buf
    local len = ffi_new("unsigned int[1]")
    if ossl.EVP_SignFinal(md_ctx, self.buf, len, self.pkey) <= 0 then
        return err()
    end
...
function _M.verify(self, str, sig)
    ...
    if ossl.EVP_VerifyFinal(md_ctx, buf, siglen, self.pkey) <= 0 then
        return err()
    end
